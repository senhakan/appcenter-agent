...
func TestApplyIfPendingForceAllowsSameVersion(t *testing.T) {
	tmp := t.TempDir()
	content := []byte("same-version-force")
	sum := sha256.Sum256(content)
	hash := fmt.Sprintf("sha256:%x", sum[:])
	newExe := filepath.Join(tmp, "agent-update-1.0.0.exe")
	if err := os.WriteFile(newExe, content, 0o600); err != nil {
		t.Fatalf("write new exe: %v", err)
	}

	meta := StagedUpdate{Version: "1.0.0", FilePath: newExe, Hash: hash, Force: true}
	metaPath := filepath.Join(tmp, "pending_update.json")
	b, _ := json.Marshal(meta)
	if err := os.WriteFile(metaPath, b, 0o600); err != nil {
		t.Fatalf("write meta: %v", err)
	...
	if r.cmd == nil {
		t.Fatalf("helper cmd not started")
	}
}
